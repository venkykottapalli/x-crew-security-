...
module.exports = {
    ...
    run: async (client, message) => {
        ...
        if (!message.guild.members.me.permissions.has(PermissionFlagsBits.ManageChannels)) {
            return message.channel.send(reply(`${client.emoji.cross} I need \`Manage Channels\` permission to unhide channels.`));
        }

        const loadingMsg = await message.channel.send(reply(`${client.emoji.loading} Unhiding all channels...`));
        ...
        return loadingMsg.edit(reply(`${client.emoji.tick} Unhidden **${success}** channels. Failed: **${failed}**`, 0x57F287));
    },
};
