...
module.exports = {
    ...
    run: async (client, message) => {
        ...
        if (!message.guild.members.me.permissions.has(PermissionFlagsBits.ManageChannels)) {
            return message.channel.send(reply(`${client.emoji.cross} I need \`Manage Channels\` permission to lock channels.`));
        }

        const loadingMsg = await message.channel.send(reply(`${client.emoji.loading} Locking all channels...`));
        ...
        return loadingMsg.edit(reply(`${client.emoji.tick} Locked **${success}** channels. Failed: **${failed}**`, 0x57F287));
    },
};
